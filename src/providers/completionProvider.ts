...
const KEYWORD_DATABASE: KeywordInfo[] = [
    // Variables
...
const CATEGORY_ICONS: { [key: string]: vscode.CompletionItemKind } = {
        ...
        provideCompletionItems(document, position, token, context) {
            ...
            for (const keyword of KEYWORD_DATABASE) {
                const item = new vscode.CompletionItem(
                    keyword.bangla,
                    CATEGORY_ICONS[keyword.category] || vscode.CompletionItemKind.Keyword
                );

                item.detail = `${keyword.bangla} → ${keyword.javascript}`;
                item.documentation = new vscode.MarkdownString(
                    `**${keyword.bangla}** (\`${keyword.javascript}\`)\n\n${keyword.description}\n\n${keyword.detail || ''}`
                );
...
export function createSignatureHelpProvider(): vscode.SignatureHelpProvider {
    return {
        provideSignatureHelp(document, position, token, context) {
            ...
            const signatures: { [key: string]: vscode.SignatureHelp } = {};
            ...
            if (linePrefix.includes('লিখো(')) {
                ...
                const sig = new vscode.SignatureInformation(
                    'লিখো(...বার্তাসমূহ)',
                    'কনসোলে বার্তা প্রিন্ট করে (console.log)'
                );
                sig.parameters = [
