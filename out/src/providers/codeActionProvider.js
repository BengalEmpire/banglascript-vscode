...
exports.createCodeActionProvider = createCodeActionProvider;
exports.registerConvertLineCommand = registerConvertLineCommand;
exports.registerConvertDocumentCommand = registerConvertDocumentCommand;
        ...
        provideCodeActions(document, range, context, token) {
            ...
            for (const [jsKeyword, banglaKeyword] of Object.entries(JS_TO_BANGLA)) {
                ...
                while ((match = regex.exec(lineText)) !== null) {
                    ...
                    if (range.intersection(matchRange)) {
                        const action = new vscode.CodeAction(`"${jsKeyword}" কে "${banglaKeyword}" এ রূপান্তর করুন`, vscode.CodeActionKind.QuickFix);
            ...
            for (const [typo, correction] of Object.entries(TYPO_CORRECTIONS)) {
                ...
                while ((match = regex.exec(lineText)) !== null) {
                    ...
                    const matchRange = new vscode.Range(startPos, endPos);
                    if (range.intersection(matchRange)) {
                        const action = new vscode.CodeAction(`"${typo}" কে "${correction}" দিয়ে সংশোধন করুন`, vscode.CodeActionKind.QuickFix);
                        action.edit = new vscode.WorkspaceEdit();
                        action.edit.replace(document.uri, matchRange, correction);
                        action.isPreferred = true;
                        action.diagnostics = context.diagnostics.filter(d => d.range.intersection(matchRange) !== undefined);
                        codeActions.push(action);
                    }
                }
            }
            ...
            if (hasJsKeywords) {
                const convertAllAction = new vscode.CodeAction('এই লাইনের সব JavaScript কীওয়ার্ড বাংলায় রূপান্তর করুন', vscode.CodeActionKind.QuickFix);
...
function registerConvertLineCommand() {
    return vscode.commands.registerCommand('banglascript.convertLineToLangla', async (document, lineNumber) => {
...
function registerConvertDocumentCommand() {
    return vscode.commands.registerCommand('banglascript.convertDocumentToBangla', async () => {
        ...
        if (replacementCount > 0) {
            ...
            await vscode.workspace.applyEdit(edit);
            vscode.window.showInformationMessage(`✅ ${replacementCount}টি JavaScript কীওয়ার্ড বাংলায় রূপান্তরিত হয়েছে!`);
        }
        else {
