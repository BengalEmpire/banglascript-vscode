...
exports.tokenizePreserve = tokenizePreserve;
...
function tokenizePreserve(code) {
    ...
    while (i < code.length) {
        ...
        if (/[\d\u09E6-\u09EF.]/.test(char)) {
            const start = i;
            while (i < code.length && /[\d\u09E6-\u09EF.]/.test(code[i]))
                i++;
        ...
        if (/[a-zA-Z\u0980-\u09FF_]/.test(char)) {
            const start = i;
            while (i < code.length && /[a-zA-Z\u0980-\u09FF0-9\u09E6-\u09EF_]/.test(code[i]))
                i++;
            const word = code.slice(start, i).normalize('NFC');
            tokens.push({ type: 'word', text: word });
            continue;
        }
        // Operator: Common JS/Bangla ops (FIXED: Escaped ] and reordered class)
        if (/[+\\-*/=<>!&|~^%=(){};,\\[\\]]/.test(char)) { // ← FIXED: \\[ \\] for [ and ], no invalid range
